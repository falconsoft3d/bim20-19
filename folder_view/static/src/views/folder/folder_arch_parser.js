...
export class GroupFolderArchParser {
    parse(arch, models, modelName, jsClass) {
        ...
        visitXML(arch, (node) => {
            if (node.tagName === "button") {
                ...
                const fieldInfo = Field.parseFieldNode(node, models, modelName, "folder", jsClass);
...
export class FolderArchParser {
    parseFieldNode(node, models, modelName) {
        return Field.parseFieldNode(node, models, modelName, "folder");
    }
    ...
    parse(xmlDoc, models, modelName) {
        ...
        let headerButtons = [];
        const creates = [];
        const groupFolderArchParser = new GroupFolderArchParser();
        let buttonGroup;
        ...
        visitXML(xmlDoc, (node) => {
            ...
            if (node.tagName === "button") {
                ...
                const groupByArchInfo = groupFolderArchParser.parse(node, models, coModelName);
                ...
                treeAttr.editable = false;
                treeAttr.multiEdit = false;
                treeAttr.openFormView = true;
                treeAttr.parentField = xmlDoc.getAttribute('parent_field') || 'parent_id';
