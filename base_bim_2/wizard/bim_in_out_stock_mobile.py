...
class BimIOStockMobile(models.TransientModel):
    ...
    def action_load(self):
        ...
        if not picking_type:
            warehouse = self.env['stock.warehouse'].search([('partner_id','=',company.partner_id.id)],limit=1)
            picking_type = pick_type_obj.search([('warehouse_id','=',warehouse.id),('code','=',code_type)],limit=1)
        ...
        if company.validate_stock:
            ...
            for move in picking.move_ids.filtered(lambda m: m.state not in ['done', 'cancel']):
                for move_line in move.move_line_ids:
                    move_line.quantity = move_line.product_uom_qty if move_line.product_id.tracking == 'none' else move_line.move_id.product_uom_qty
            """
...
class BimIOStockMobileLines(models.TransientModel):
    ...
    qty_add = fields.Float('Quantity to move', required=True, default=0.0, digits="BIM qty")
