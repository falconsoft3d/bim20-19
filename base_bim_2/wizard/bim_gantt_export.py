...
class BimGanttExport(models.TransientModel):
    ...
    def print_xml(self):
        if self.gantt_type == 'xls':
            return self.generate_xls_report()
        elif self.gantt_type == 'xls2':
            return self.generate_xls2_report()
        elif self.gantt_type == 'ms':
            xml = self.generate_ms_project()
        elif self.gantt_type == 'csv':
            return self.exe_export_csv()
        else:
    ...
    def generate_ms_project(self):
        ...
        _date_from = self.budget_id.date_from if self.budget_id.date_from else self.budget_id.project_id.date_start
        _date_to = self.budget_id.date_to  if self.budget_id.date_to else self.budget_id.project_id.date_end

        _start = datetime.datetime.combine(_date_from, datetime.time(9, 0)).strftime(dt_format)
        _end = datetime.datetime.combine(_date_to, datetime.time(9, 0)).strftime(dt_format)
    ...
    def generate_xls2_report(self):
        budget = self.budget_id
        workbook = xlwt.Workbook(encoding="utf-8")
        worksheet = workbook.add_sheet("%s - %s"%(_("Budget"),budget.code))
        file_name = "%s - %s"%(_("Budget"),budget.code)
        style_title = xlwt.easyxf(
            'font: name Times New Roman 180, color-index black, bold on; align: wrap yes, horiz center')
        style_border_table_top = xlwt.easyxf(
            'borders: left thin, right thin, top thin, bottom thin; font: bold on;')
        style_border_table_details_right = xlwt.easyxf('borders: bottom thin;align: wrap yes, horiz right')
        style_border_table_details_chapter_left = xlwt.easyxf('pattern: pattern solid, fore_colour light_green; borders: bottom thin;align: wrap yes, horiz left')
        style_border_table_details_chapter_left.num_format_str = '$#,##0.00'
        style_border_table_details = xlwt.easyxf('borders: bottom thin;align: wrap yes')
        style_border_table_details.num_format_str = '$#,##0.00'

        worksheet.write_merge(0, 0, 0, 0, "Code", style_border_table_top)
        worksheet.write_merge(0, 0, 1, 5, "Concept", style_border_table_top)
        worksheet.write_merge(0, 0, 6, 6, "Type", style_border_table_top)
        worksheet.write_merge(0, 0, 7, 7, "Begin", style_border_table_top)
        worksheet.write_merge(0, 0, 8, 8, "End", style_border_table_top)

        chapters = self.budget_id.concept_ids.filtered_domain([('type','=','chapter')])
        row = 1
        for chapter in chapters:
            worksheet.write_merge(row, row, 0, 0, chapter.code, style_border_table_details_chapter_left)
            worksheet.write_merge(row, row, 1, 5, chapter.name, style_border_table_details_chapter_left)
            worksheet.write_merge(row, row, 6, 6, "Chapter", style_border_table_details_chapter_left)
            worksheet.write_merge(row, row, 7, 7, chapter.acs_date_start.strftime('%d/%m/%Y') if chapter.acs_date_start else '',style_border_table_details_chapter_left)
            worksheet.write_merge(row, row, 8, 8, chapter.acs_date_end.strftime('%d/%m/%Y') if chapter.acs_date_end else '',style_border_table_details_chapter_left)
            row += 1
            for departure in chapter.child_ids.filtered_domain([('type','=','departure')]):
                worksheet.write_merge(row, row, 0, 0, departure.code, style_border_table_details_right)
                worksheet.write_merge(row, row, 1, 5, departure.name, style_border_table_details)
                worksheet.write_merge(row, row, 6, 6, "Departure", style_border_table_details)
                worksheet.write_merge(row, row, 7, 7, departure.acs_date_start.strftime('%d/%m/%Y') if departure.acs_date_start else '',
                                      style_border_table_details)
                worksheet.write_merge(row, row, 8, 8, departure.acs_date_end.strftime('%d/%m/%Y') if departure.acs_date_end else '',
                                      style_border_table_details)
                row += 1
        fp = BytesIO()
        workbook.save(fp)
    ...
    def exe_export_csv(self):
        data = "%s,%s,%s,%s,%s\n"%(_("Code"),_("Concept"),_("Type"),_("Date Start"),_("Date End"))
        chapters = self.budget_id.concept_ids.filtered_domain([('type', '=', 'chapter')])
        for chapter in chapters:
            data += '%s,%s,%s,%s,%s\n' % (
                chapter.code or '',
                chapter.name.replace(',','-').replace(';','-') or '',
                _("Chapter"),
                chapter.acs_date_start.strftime('%d-%m-%Y') if chapter.acs_date_start else '',
                chapter.acs_date_end.strftime('%d-%m-%Y') if chapter.acs_date_end else ''
            )
            for departure in chapter.child_ids.filtered_domain([('type', '=', 'departure')]):
                data += '%s,%s,%s,%s,%s\n' % (
                    departure.code or '',
                    departure.name.replace(',','-').replace(';','-') or '',
                    _("Departure"),
                    departure.acs_date_start.strftime('%d-%m-%Y') if departure.acs_date_start else '',
                    departure.acs_date_end.strftime('%d-%m-%Y') if departure.acs_date_end else ''
                )

        file_name = '%s - %s.csv'%(_("Budget"),self.budget_id.id)
        data_b64 = base64.encodebytes(data.encode('utf-8'))
        doc = self.env['ir.attachment'].create({
            'name': file_name,
            'datas': data_b64,
            'store_fname': 'documentos.csv',
        })
        return {
            'name': file_name,
            'type': 'ir.actions.act_url',
            'url': 'web/content/%d?download=true' % doc.id,
            'target': 'self',
        }
