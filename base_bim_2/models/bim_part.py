...
class BimPart(models.Model):
    ...
    date = fields.Date(string='Date', required=True, readonly=True, copy=False, default=fields.Datetime.now)
    budget_id = fields.Many2one('bim.budget', string='Budget', domain="[('project_id','=',project_id),('state_id.project_part','=',True)]")
    ...
    part_total = fields.Float(compute='_compute_part_total', digits="BIM price")
    ...
    all_lines_purchase = fields.Boolean(compute='_compute_all_lines_purchase')

    def _compute_all_lines_purchase(self):
        for part in self:
            part.all_lines_purchase = all(line.purchase for line in part.lines_ids)
    ...
    def create_purchase_order(self):
        ...
        for supplier in suppliers:
            ...
            if purchase_now:
                ...
                raise UserError(_('All lines are already purchased'))
