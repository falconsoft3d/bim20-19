...
class BimWorkOrder(models.Model):
    ...
    date_order = fields.Datetime(string='Date Order', required=True, readonly=True, index=True,
                                 copy=False, default=fields.Datetime.now, help="Creation date of draft/sent orders,\nConfirmation date of confirmed orders.")
    validity_date = fields.Date(string='Validity Date', readonly=True, copy=False,
    ...
    order_line = fields.One2many('bim.work.order.line', 'order_id', string='Order Lines',  copy=True, auto_join=True)
    ...
    amount_untaxed = fields.Monetary(string='Tax Base', store=True, readonly=True, compute='_amount_all', tracking=True)
    ...
    amount_discount = fields.Float('Discount', compute='_compute_amount_undiscounted', digits=0)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            # Asegurar partner_invoice_id, partner_shipping_id y pricelist_id
            if any(f not in vals for f in ['partner_invoice_id', 'partner_shipping_id', 'pricelist_id']):
                partner = self.env['res.partner'].browse(vals.get('partner_id'))
                if partner:
                    addr = partner.address_get(['delivery', 'invoice'])
                    vals.setdefault('partner_invoice_id', addr.get('invoice'))
                    vals.setdefault('partner_shipping_id', addr.get('delivery'))
                    vals.setdefault(
                        'pricelist_id',
                        partner.property_product_pricelist.id if partner.property_product_pricelist else False
                    )
...
class BimWorkOrderLine(models.Model):
    ...
    price_unit = fields.Float('Price', required=True, default=0.0, digits="BIM price")

    price_subtotal = fields.Monetary(compute='_compute_amount', string='Subtotal', readonly=True, store=True)
    price_tax = fields.Float(compute='_compute_amount', string='Total Tax', readonly=True, store=True, digits="BIM price")
    price_total = fields.Monetary(compute='_compute_amount', string='Total', readonly=True, store=True)

    price_reduce = fields.Float(compute='_get_price_reduce', string='Price Reduce', digits="BIM price", readonly=True, store=True)
    ...
    def _get_display_price(self, product):
        ...
        final_price, rule_id = self.order_id.pricelist_id.with_context(product_context)._get_product_price_rule(self.product_id, self.product_uom_qty or 1.0)
        ...
        if rule_id:
            ...
            if pricelist_item.pricelist_id.discount_policy == 'without_discount':
                while pricelist_item.base == 'pricelist' and pricelist_item.base_pricelist_id and pricelist_item.base_pricelist_id.discount_policy == 'without_discount':
                    price, rule_id = pricelist_item.base_pricelist_id.with_context(uom=uom.id)._get_product_price_rule(product, qty)
                    pricelist_item = PricelistItem.browse(rule_id)
    ...
    def _onchange_discount(self):
        ...
        price, rule_id = self.order_id.pricelist_id.with_context(product_context)._get_product_price_rule(self.product_id, self.product_uom_qty or 1.0)
