    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('bim.master.checklist') or 'New'
...
class BimChecklist(models.Model):
    ...
    obs = fields.Text(string="Observations", default="")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('bim.checklist') or 'New'
        return super().create(vals_list)
    ...
    def action_checklist_send(self):
        ...
        self.ensure_one()
        try:
            template_id = self.env.ref('base_bim_2.email_template_checklist').id or False
        except ValueError:
            template_id = False
        try:
            compose_form_id = self.env.ref('mail.email_compose_message_wizard_form').id or False
        except ValueError:
...
class BimChecklistNotes(models.Model):
    ...
    name = fields.Text(string='Description', default="")
