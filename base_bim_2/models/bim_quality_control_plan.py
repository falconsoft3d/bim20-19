...
class BimQualityControlPlanLine(models.Model):
    ...
    activity = fields.Text(string="Activity", required=True, default="")
    inspection_type_id = fields.Many2one('bim.inspection.type', ondelete="restrict", required=True)
    user_id = fields.Many2one('res.users', string='Responsible', default=lambda self: self.env.user)
    characteristic = fields.Text(string="Characteristic", required=True, default="")
    frequency = fields.Char(string="Frequency", required=True)
    criterion = fields.Text(string="Criterion", required=True, default="")
    reference = fields.Text(string="Reference", default="")
    ...
    register_related = fields.Text(default="")
    inspection_ime = fields.Text(default="")
